...
if($Config["accent"] != "none" && array_key_exists($Config["accent"], $Accent_Colors))
{
...
?>
	...
	var GamemodeNames = <?php echo json_encode($Config["gamemode_names"]); ?>;
	</script>
...
<body>
	...
	if(!array_key_exists("disable", $Config["audio"]) || !$Config["audio"]["disable"])
	{
		// YouTube URL
		$matches = array();
		if(preg_match("/(youtu\.be\/|youtube\.com\/(watch\?(.*&)?v=|(embed|v)\/))([^\?&\"'>]+)/", $Audio_File["path"], $matches))
		{
			$id = $matches[5];
?>
